...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            # shape (num_classes, num_support, latents)
            support = self._network(images_support)
            queries = self._network(images_query)
            prototypes = torch.stack([torch.mean(support[labels_support == i], dim=0) 
                                      for i in range(support.shape[0])]) # calculate the mean of each class
            query_pairwise_distances = torch.sum((queries.unsqueeze(1) - prototypes.unsqueeze(0)) ** 2, dim=2) # calculate the squared distance between each query and each prototype
            # query_pairwise_distances = torch.cdist(queries, prototypes) # calculate the distance between each query and each prototype
            support_pairwise_distances = torch.sum((support.unsqueeze(1) - prototypes.unsqueeze(0)) ** 2, dim=2) # calculate the squared distance between each support and each prototype
            logits = -query_pairwise_distances # the logits are the negative distance
            loss = F.cross_entropy(logits, labels_query) # calculate the cross entropy loss
            loss_batch.append(loss)
            query_predicted_clases = torch.argmin(query_pairwise_distances, dim=1) # the class is the one with the minimum distance
            support_predicted_clases = torch.argmin(support_pairwise_distances, dim=1) # the class is the one with the minimum distance
            accuracy_support = torch.mean((support_predicted_clases == labels_support).float()) # calculate the accuracy
            accuracy_query = torch.mean((query_predicted_clases == labels_query).float()) # calculate the accuracy
            accuracy_support_batch.append(accuracy_support) # append the accuracy
            accuracy_query_batch.append(accuracy_query) # append the accuracy
