...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        } # copy of meta_parameters
        ### START CODE HERE ###
        for i in range(self._num_inner_steps + 1):
            logits = self._forward(images, parameters) # Logits of the support set
            predictions = torch.argmax(logits, dim=1) # Predictions of the support set
            accuracy = torch.mean((predictions == labels).float()) # Accuracy of the support set
            accuracies.append(accuracy.item()) # Append the accuracy to the list

            loss = F.cross_entropy(logits, labels) # Compute the loss
            grad = autograd.grad(loss, parameters.values(), create_graph=train) # create_graph=True for higher order derivatives
            
            # Update parameters
            # Update the inner loop parameters with the inner learning rates
            for i, (k, v) in enumerate(parameters.items()):
                parameters[k] = v - self._inner_lrs[k] * grad[i]
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            ### START CODE HERE ###
            parameters, accuracies_support = self._inner_loop(images_support, 
                                                              labels_support, 
                                                              train) # Compute the inner loop parameters and accuracies
            accuracies_support_batch.append(accuracies_support) # Append the accuracies to the list
            logits = self._forward(images_query, parameters) # Compute the logits of the query set
            print("logits.shape:",logits.shape)
            predictions = torch.argmax(logits, dim=1) # Compute the predictions of the query set
            print("predictions.shape:",predictions.shape)
            print("labels_query.shape:",labels_query.shape)
            print("predictions:",predictions)
            print("labels_query:",labels_query)
            print("predictions == labels_query:",torch.mean((predictions == labels_query).float()))
            accuracy_query = torch.mean((predictions == labels_query).float()) # Compute the accuracy of the query set
            print("accuracy_query.shape:",accuracy_query.shape)
            accuracy_query_batch.append(accuracy_query.item()) # Append the accuracy to the list
            outer_loss = F.cross_entropy(logits, labels_query) # Compute the outer loss
            print("accuracy_query.shape:",outer_loss.shape)
            outer_loss_batch.append(outer_loss) # Append the outer loss to the list
            self._optimizer.zero_grad() # Zero the gradients
            outer_loss.backward() # Backpropagate the gradients
